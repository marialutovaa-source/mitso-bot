...
def format_schedule(lessons: list[dict]) -> str:
    ...
    return "\n".join(lines).strip()                subject     = parts[0]
                lesson_type = ""
                room        = ""
                teacher     = ""
            lessons.append({
...
def format_schedule(lessons: list[dict]) -> str:
    ...
    return "\n".join(lines).strip()            parts = [p.strip() for p in rest.split('|')]
...
def format_schedule(lessons: list[dict]) -> str:
    if not lessons:
        return "Расписание не загружено. Перешлите сообщение от @mitsoScheldueBot старосте командой /setschedule"
    ...
    for item in lessons:
        ...
        room = f" | {item['room']}" if item['room'] else ""
        teacher = f" | {item['teacher']}" if item['teacher'] else ""
        lines.append(f"  `{item['time']}` {item['type']}{room} — *{item['subject']}*{teacher}")
    return "\n".join(lines).strip()

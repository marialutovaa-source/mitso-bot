...
async def generate_student_report(student_id: int) -> io.BytesIO:
    ...
    doc.save(buf)
    buf.seek(0)
    return buf    period_str = " ".join(period_parts) if period_parts else "за всё время"
...
async def generate_student_report(student_id: int) -> io.BytesIO:
    ...
    doc.save(buf)
    buf.seek(0)
    return buf        period_parts.append(f"по {date_to.strftime('%d.%m.%Y')}")
    period_str = " ".join(period_parts) if period_parts else "за всё время"
    ...
    hdr = table.rows[0].cells
    headers = ["№", "Студент", "Дисциплина", "Часов пропущено"]
    for i, text in enumerate(headers):
        hdr[i].text = text
        hdr[i].paragraphs[0].runs[0].bold = True
        hdr[i].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    for student in students:
        ...
        for r in rows:
            by_disc[r["discipline"]] += r["hours"]

        for disc_name, hours in sorted(by_disc.items()):
            row_cells = table.add_row().cells
            row_cells[0].text = str(n)
            row_cells[1].text = f"{student['last_name']} {student['first_name']}"
            row_cells[2].text = disc_name
            row_cells[3].text = str(hours)
            for cell in row_cells:
                cell.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
            row_cells[1].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.LEFT
            row_cells[2].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.LEFT
    ...
    total_row[0].paragraphs[0].runs[0].bold = True
    total_row[0].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.RIGHT
    total_row[3].text = str(total_hours)
...
async def generate_student_report(student_id: int) -> io.BytesIO:
    ...
    hdr = table.rows[0].cells
    for i, text in enumerate(["Дата", "Дисциплина", "Часов", "Уважительная"]):
        hdr[i].text = text
        hdr[i].paragraphs[0].runs[0].bold = True
        hdr[i].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    for row in absences:
        ...
        r[0].text = row["date"].strftime("%d.%m.%Y")
        r[1].text = row["discipline"]
        r[2].text = str(row["hours"])
        r[3].text = "✓" if row["confirmed"] else "—"
        for c in r:
            c.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
        r[1].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.LEFT
        total += row["hours"]
    ...
    tr = table.add_row().cells
    tr[0].merge(tr[1]).merge(tr[2])
    tr[0].text = "Итого:"
    tr[0].paragraphs[0].runs[0].bold = True
    tr[0].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.RIGHT
    tr[3].text = str(total)
    tr[3].paragraphs[0].runs[0].bold = True
    tr[3].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
